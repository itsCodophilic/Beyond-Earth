...
# towards the camera.
BODIES = {
    # Vanth: a video still. The body's left third is terminator shadow, so the
    # limb fit is pulled towards the lit side; the illumination fit removes the
    # shading and the four readings cover what the shadow does not.
    "vanth":   dict(cx=222.1, cy=124.1, A=101.1, B=101.1, ang=0.0, sub=0.0,
                    lit=(0.30, 0.62), dark=(0.07, 0.17)),
    "xiangliu": dict(cx=126.3, cy=121.8, A=120.0, B=120.0, ang=0.0, sub=0.0),
    # Both Haumea moons come out of one comparison illustration, where they are
    # 66 and 39 pixels across. Upscaling first keeps the projection from
    # sampling a staircase; it cannot add detail that was never there.
    "hi_iaka": dict(cx=135.3, cy=490.7, A=66.2, B=66.2, ang=0.0, sub=0.0,
                    source="haumea_family", upscale=4.0),
    "namaka":  dict(cx=323.7, cy=507.7, A=39.0, B=39.0, ang=0.0, sub=0.0,
                    source="haumea_family", upscale=6.0),
}
...
def build_from_patch(name, p):
    img = np.asarray(Image.open(f"/tmp/dw/src/{name}.png").convert("RGB")).astype(np.float32) / 255
    x0, y0, x1, y1 = p["ice"]
...
def load_source(name, p):
    """One image can hold more than one moon, and a moon can be too small to
    project directly. Both are handled here so the rest of the pipeline never
    has to know."""
    path = f"reference/{p.get('source', name)}.png"
    img = Image.open(path).convert("RGB")
    scale = p.get("upscale", 1.0)
    if scale != 1.0:
        img = img.resize((int(img.width * scale), int(img.height * scale)), Image.LANCZOS)
        for key in ("cx", "cy", "A", "B"):
            p[key] = p[key] * scale
